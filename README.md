...
│
├── lambdas/
│   ├── youtube_api_integstion/        # Ingestion Lambda (Fetches trending videos & categories)
│   │   └── lambda_function.py         
...
│
├── data/                              # Reference & historical Kaggle data
...
Data is stored as raw JSON in S3, partitioned by region, date, and hour. Historical Kaggle CSV data can also be uploaded via the provided `aws_copy.sh` script.

### Silver Layer (Cleansed Data)
Two parallel transformations run on Bronze data:
1. **Statistics (Glue Job: `bronze_to_silver_statistics`)**: Schema enforcement, type casting (views, likes, dislikes → Long; dates parsed), null handling, deduplication (latest record per video/region/date), and derivation of metrics (`like_ratio`, `engagement_rate`). Output is Parquet with Snappy compression, partitioned by region.
2. **Reference Data (Lambda: `json_to_parquet`)**: Normalizes JSON category mappings to tabular format and deduplicates category entries. Output is Parquet, partitioned by region.

### Data Quality Gate
Before data moves to Gold, a Lambda function (`dq_lambda`) validates the Silver data based on:
- **Row count** (>= 10 rows)
- **Null percentages** (<= 5% on critical columns)
...
A Glue job (`silver_to_gold_analytics.py`) produces three analytics tables from cleansed Silver data.
...
*(Note: See `scripts/information.md` for Dev environment specifics like `yt-data-pipeline-bronze-ap-south-1-dev`.)*
